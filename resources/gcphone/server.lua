...
function getNumberPhone(identifier)
	...
	if result[1] ~= nil then
		print('trouvé '..identifier..' : '..result[1].telephone)
...
function getIdentifierByPhoneNumber(phone)
    local result = MySQL.Sync.fetchAll("SELECT users.identifier FROM users WHERE users.telephone = @phone", {
        ['@phone'] = phone
    })
    if result[1] ~= nil then
...
function getMessages(identifier)
    -- local result = MySQL.Sync.fetchAll("SELECT phone_messages.* FROM phone_messages LEFT JOIN users ON users.identifier = @identifier WHERE phone_messages.receiver = users.phone", {
    --     ['@identifier'] = identifier
    -- })
    -- -- A CHANGER !!!!!!!
    ...
    -- end
    -- return result
    return MySQLQueryTimeStamp("SELECT phone_messages.* FROM phone_messages LEFT JOIN users ON users.identifier = @identifier WHERE phone_messages.receiver = users.telephone", {
        ['@identifier'] = identifier
    })
end
...
function addMessage(source, identifier, phone, message)
    ...
    if otherIdentifier ~= nil then 
        ...
        local osou = MySQL.Sync.fetchAll("SELECT users.session_id FROM users WHERE users.identifier = @identifier", {
        ['@identifier'] = otherIdentifier
		})
            if osou[1].session_id ~= nil then 
                --TriggerClientEvent("gcPhone:allMessage", osou[1].session_id, getMessages(otherIdentifier))
                print('Jenvoie à '..osou[1].session_id)
				TriggerClientEvent("gcPhone:receiveMessage", osou[1].session_id, tomess)
            end
    end
