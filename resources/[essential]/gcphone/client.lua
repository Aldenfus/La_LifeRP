...
RegisterNUICallback('callEvent', function(data, cb)
  ...
  if data.eventName ~= 'cancel' then
    if data.data ~= nil then 
      ...
      TriggerServerEvent("call:makeCall", data.eventName, {x=plyPos.x,y=plyPos.y,z=plyPos.z}, data.data, GetPlayerServerId(GetPlayerPed(-1)))
      ...
      if data.eventName ~= "RESPAWN" then
        ...
        TriggerServerEvent("call:makeCall", data.eventName, {x=plyPos.x,y=plyPos.y,z=plyPos.z}, text, GetPlayerServerId(GetPlayerPed(-1)))
        if data.eventName == "police" then
          ShowNotificationMenuCivil2("~h~Vous avez appelé la ~b~POLICE")
        elseif data.eventName == "taxi" then
          ShowNotificationMenuCivil2("~h~Vous avez appelé un ~b~TAXI")
